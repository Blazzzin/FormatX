...
function handleFileSelect(event) {
    ...
    files.forEach(file => {
        if (!filesList.some(existingFile => existingFile.name === file.name)) {
            ...
            icon.src = '../static/assets/word-icon.png';
            icon.alt = 'Word Icon';
...
document.getElementById('wordToPDFForm').addEventListener('submit', async function (event) {
    ...
    const fetchPromise = fetch(`${API_BASE_URL}/convert/word-to-pdf`, {
        method: 'POST',
        headers: headers,
        body: formData
